...
correctax=axes('parent',correctfig','position',[ 0.0500    0.1500    0.8750    0.77]);
...
%build the buttons
for i=1:length(typeslist);  % type buttons
    displayplothandle(i)=uicontrol('parent',correctfig,'style','togglebutton','units','normalized',...
        'position',[.906 .90-i*.0401 .06 .04],...
...
for j=1:length(specieslist);
    displayplothandle(i+j)=uicontrol('parent',correctfig,'style','togglebutton','units','normalized',...
        'position',[.906 .90-.05-(i+j)*.0401 .06 .04],...
    ...
    %         'string',fittypes,'callback',@fitchange,'userdata',{'species' species{j}},'value',1);
        dividebyhandle(j)=uicontrol('parent',correctfig,'style','radiobutton','units','normalized',...
            'position',[.906+.063 .90-.05-(i+j)*.0401+.01 .02 .02],...
        'string', '','callback',@divideby,'userdata',{'species' specieslist{j}},'value',0,'backgroundcolor',[.8 .8 .8]);

end


uicontrol('parent',correctfig,'style','text','units','normalized','position',[.1 .055 .1 .04],...
    'string','Blank Correction','backgroundcolor',[.8 .8 .8]);
subtractblankhdl=uicontrol('parent',correctfig,'style','popupmenu','units','normalized','position',[.1 .04 .1 .04],...
    'string',{'None','Linear','Nearest Neighbor'},'callback',@subtractblank,'value',1,'backgroundcolor',[.8 .8 .8]);

outputhandle=uicontrol('parent',correctfig,'style','pushbutton','units','normalized','position',[.8 .03 .1 .04],...
    'string','Output Data','backgroundcolor',[.8 .8 .8],'callback',@outputdata);

displayplot([],[]);

    function divideby(src,event); % handle radio button changes for divide by function;
        v=get(src,'value');
        set(dividebyhandle,'value',0);
        if v;set(src,'value',1);end
    ...
    function displayplot(src,event);
        ...
        divideby=cell2mat(get(dividebyhandle,{'value'}));
        if any(divideby);
            ud=get(dividebyhandle(find(divideby)),{'userdata'});
             divideby=ud{1}{2};
        else
            divideby=[];
        end
        ...
        for ti=1:length(types)
            for s=1:length(species)
                ...
                cs=[data.(types{ti}).(speciesname).color data.(types{ti}).(speciesname).symbol];
               % disp([types{ti} speciesname ystring])
                ...
                dtemp=data.(types{ti}).(speciesname).(ystring).result; 
                etemp=data.(types{ti}).(speciesname).(ystring).error;
                if ~isempty(divideby);
                 
                   etemp=(dtemp./data.(types{ti}).(divideby).(ystring).result).*sqrt((data.(types{ti}).(speciesname).(ystring).error./dtemp).^2 ...
                             +(data.(types{ti}).(divideby).(ystring).error./data.(types{ti}).(divideby).(ystring).result).^2);
                                        dtemp=dtemp./data.(types{ti}).(divideby).(ystring).result;  
                end
                
                %newfit=1
                if  ~newfit && isfield(data.(types{ti}).(speciesname).(ystring),'fit') && ~isempty( data.(types{ti}).(speciesname).(ystring).fit);
                else
           ...
           %         data.(types{ti}).(speciesname).(ystring).fit=makefit(data.(types{ti}).(speciesname).time(mask),data.(types{ti}).(speciesname).(ystring).result(mask),fittype);
                   data.(types{ti}).(speciesname).(ystring).fit=makefit(data.(types{ti}).(speciesname).time(mask),dtemp(mask),fittype);
  
                    data.(types{ti}).(speciesname).(ystring).fit.type=types{ti};data.(types{ti}).(speciesname).(ystring).fit.species=speciesname;
                end
                
                errorbarhandle=errorbar(data.(types{ti}).(speciesname).time,dtemp,etemp,cs );  %%
                errorbar_tick(errorbarhandle,.01,'units');
                
                plotfit(data.(types{ti}).(speciesname).(ystring).fit,data.(types{ti}).(speciesname).color,correctax);
                
                hold on
                plot(data.(types{ti}).(speciesname).time(mask),dtemp(mask),'linestyle','none','marker',cs(2),'markerfacecolor',cs(1),...
                    'buttondownfcn',@plotcallback,'userdata',data.(types{ti}).(speciesname).(ystring).fit);
                if ~all(mask);
                    plot(data.(types{ti}).(speciesname).time(~mask),dtemp(~mask),'linestyle','none','marker',cs(2),'markerfacecolor',[.1 .1 .1],...
                        'buttondownfcn',@plotcallback,'userdata',data.(types{ti}).(speciesname).(ystring).fit);
                end
        ...
        newfit=0;
        if ~isempty(xticks)
            set(correctax,'position',[ 0.0500    0.1500    0.8750    0.77]);
        ...
        switch get(gcf,'selectiontype');
            case 'normal'
                ...
                if sum(mask)>1;
                    ...
                    data.(fitdata.type).(fitdata.species).(ystring).fit=makefit(data.(fitdata.type).(fitdata.species).time(mask),data.(fitdata.type).(fitdata.species).(ystring).result(mask),data.(fitdata.type).(fitdata.species).(ystring).fit.fittype);
                    data.(fitdata.type).(fitdata.species).(ystring).fit.type=fitdata.type;data.(fitdata.type).(fitdata.species).(ystring).fit.species=fitdata.species;
        ...
        switch option
            ...
            for s=1:length(specieslist);
                ...
                for ti=1:length(typeslist);
                    ...
                    data.(typeslist{ti}).(specieslist{s}).resdeblanked.error= sqrt(data.(typeslist{ti}).(specieslist{s}).res.error.^2 + DELTA.^2);
    ...
    deblankedfiterrorcol=15;
      colheader1={'Run Id','Data File' ,'Acq Time','Run File','Number of Cycles','Sample Name','Sample Type','Material',...
          'LaserPosition','LaserPower','LaserDuration'};
      colheader2=colheader1;
      temp=repmat(1:length(specieslist),length(specieslist),1);temp=temp(:)';
      colheader1=[colheader1 specieslist(temp)'];
      
      for i=1:length(specieslist);colheader2=[colheader2  {'Fit Result','Fit Error','Corrected Fit','Corrected Error'}];end
                   ...
                   output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],runidentifiercol)=...
                    ...
                    output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],materialcol)=...
                      ({dbdata([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}]).material});  
                                     output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],laserposcol)=...
                      ...
                      ({dbdata([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}]).laserduration});  
                        ...
                        output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],fitresultcol+(s-1)*4)= ...
                                mat2cell(data.(typeslist{ti}).(specieslist{s}).res.result(:),ones(size(data.(typeslist{ti}).(specieslist{s}).res.result(:))));
                                   output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],fiterrorcol+(s-1)*4)= ...
                                mat2cell(data.(typeslist{ti}).(specieslist{s}).res.error(:),ones(size(data.(typeslist{ti}).(specieslist{s}).res.error(:))));
                            if ~isempty(   data.(typeslist{ti}).(specieslist{s}).resdeblanked.result);
                                
                            output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],deblankedfitrescol+(s-1)*4)= ...
                             ...
                             output([data.(typeslist{ti}).(specieslist{s}).dbdataindex{:}],deblankedfiterrorcol+(s-1)*4)= ...
